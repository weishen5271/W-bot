...
def _base_system_prompt() -> str:
    ...
    return (
        ...
        "- 用户点名 skill 时必须优先使用；否则按意图匹配最小必要 skill。命中后优先调用 skill_view(name) 加载对应说明，并在当前 Agent 内按说明执行；只有当用户明确要求隔离/并行/后台执行，或委派给子 Agent 明显更合适时，才使用 run_skill。未使用 skill 时简要说明原因。\n"
        ...
        "- 当用户引用跨会话历史（如“上次”“之前”“以前”“继续那个问题”“之前怎么修的”）或你怀疑历史会话有相关上下文时，优先使用 session_search 检索历史会话，再回答或继续执行；不要要求用户重复已经聊过的内容。\n"
        "- 工具调用参数必须严格匹配 schema。\n"
